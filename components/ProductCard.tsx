...
interface ProductCardProps {
  product: Product;
  locale: string;
  labels: ProductCardLabelsConfig;
...
export function ProductCard({
  ...
  return (
    <Card
      className={cn(
        'group border-0 bg-card shadow-sm hover:shadow-lg transition-shadow duration-300',
        className
      )}
    >
      <Link href={product.href} className="block">
        ...
        <CardContent className="p-4">
          ...
          {/* Stock Status */}
          {!product.inStock && (
            <p className="text-sm text-destructive mt-2">{labels.outOfStockLabel}</p>
          )}
        </CardContent>
      </Link>
    </Card>
